...
`
  ...
  css += '  /* soft */\n'
  allColors.forEach(colorName => {
    const lightConfig = backgroundConfig.variants.soft.light

    css += `.bu-bg-soft-${colorName} {\n`
    css += `  background-color: var(--color-${colorName}-${lightConfig.bgShade});\n`
    css += `  color: ${lightConfig.textColor};\n`
    css += `}\n`
  })
  ...
  // Dark mode overrides for soft
  allColors.forEach(colorName => {
    const darkConfig = backgroundConfig.variants.soft.dark

    css += `.b-dark .bu-bg-soft-${colorName} {\n`
    css += `  background-color: var(--color-${colorName}-${darkConfig.bgShade});\n`
    css += `  color: ${darkConfig.textColor};\n`
    css += `}\n`
  })
...
export function generateForegroundUtilities(): string {
  let css = `/*
 * Foreground Utilities - Auto-generated from design tokens
 *
 * This file is automatically generated from src/tokens/colors.ts
 * Do not edit this file directly - modify the tokens instead.
 *
 * Generated variants:
 * - .bu-fg-{color} (solid)
 * - .bu-fg-light-{color} (light)
 * - .bu-fg-lighter-{color} (lighter)
 *
 * Each variant includes dark mode overrides with .b-dark prefix.
 */
...
`
  ...
  const specialFg: Record<string, string> = {
    inherit: 'inherit',
  ...
  // Special colors
  objectEntries(specialFg).forEach(([name, value]) => {
    css += `.bu-fg-${name} {\n`
    css += `  color: ${value};\n`
    css += `}\n\n`
  })
  ...
  function writeVariant(variant: 'solid' | 'soft' | 'light' | 'lighter') {
    const prefix = variant === 'solid' ? '' : `${variant}-`

    css += `  /* ${variant} */\n`

    // Special colors for this variant (only for non-solid variants)
    if (variant !== 'solid') {
      objectEntries(specialFg).forEach(([name, value]) => {
        css += `.bu-fg-${prefix}${name} {\n`
        css += `  color: ${value};\n`
        css += `}\n`
      })
    }

    const lightShade = backgroundConfig.variants[variant].light.bgShade
    allColors.forEach(colorName => {
      css += `.bu-fg-${prefix}${colorName} {\n`
      css += `  color: var(--color-${colorName}-${lightShade});\n`
      css += `}\n`
    })

    css += `\n  /* light dark */\n`

    const darkShade = backgroundConfig.variants[variant].dark.bgShade
    objectEntries(specialFg).forEach(([name, value]) => {
      css += `.b-dark .bu-fg-${prefix}${name} {\n`
      css += `  color: ${value};\n`
      css += `}\n`
    })

    allColors.forEach(colorName => {
      css += `.b-dark .bu-fg-${prefix}${colorName} {\n`
      css += `  color: var(--color-${colorName}-${darkShade});\n`
      css += `}\n`
    })
