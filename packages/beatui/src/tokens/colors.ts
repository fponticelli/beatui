...
/**
 * Registry interface for user-defined custom color palettes.
 *
 * Augment this interface via declaration merging to make custom color names
 * available in component `color` props with full type-safety and autocomplete.
 *
 * The BeatUI Vite plugin automatically generates the declaration file when
 * `customColors` is configured, so manual augmentation is typically not needed.
 *
 * @example
 * ```ts
 * // beatui-custom-colors.d.ts (auto-generated by the Vite plugin)
...
/**
 * Union type of user-defined custom color names derived from {@link CustomColorRegistry}.
 * Evaluates to `never` when no custom colors are registered.
 */
export type CustomColorName = keyof CustomColorRegistry & string

/**
 * A record mapping each shade (50–950) to a CSS color value.
 * Used to define custom color palettes.
 *
 * @example
 * ```ts
 * const rust: ColorShadeMap = {
 ...
 * ```
 */
export type ColorShadeMap = Record<ColorShade, string>

/**
 * Utility type for registering custom color names from a `customColors` object.
 * Use with declaration merging to keep the registry in sync with your runtime config.
 *
 * @example
 * ```ts
 * import type { RegisterColors, ColorShadeMap } from '@tempots/beatui'
 * import { myColors } from './my-colors'
 *
 * declare module '@tempots/beatui' {
 *   interface CustomColorRegistry extends RegisterColors<typeof myColors> {}
 * }
 * ```
 */
export type RegisterColors<T extends Record<string, ColorShadeMap>> = {
  [K in keyof T]: true
}
 ...
 * semantic colors, custom colors registered via {@link CustomColorRegistry},
 * and the special 'black' and 'white' values.
 */
 ...
 *
 * The generic parameter `C` allows additional custom color names (inferred from
 * a `customColors` configuration) to be accepted alongside built-in palette names.
 *
 * @example
 * ```ts
 ...
 *
 * // With custom colors:
 * const overrides: SemanticColorOverrides<'rust'> = { primary: 'rust' }
 * ```
 */
export type SemanticColorOverrides<C extends string = never> = Partial<
  Record<SemanticColorName, ThemeColorName | C>
>
 ...
 * Includes `--color-white`, `--color-black`, `--color-inherit`, all
 * base palette colors at every shade level, and any user-provided custom color palettes.
 *
 * @param customColors - Optional record of custom color palettes to register
 ...
 *
 * const varsWithCustom = generateCoreColorVariables({
 *   rust: { 50: 'oklch(...)', ..., 950: 'oklch(...)' },
 * })
 * // varsWithCustom['--color-rust-500'] === 'oklch(...)'
 * ```
 */
export function generateCoreColorVariables<C extends string = never>(
  ...
  // custom colors
  if (customColors) {
    for (const [colorName, shades] of Object.entries(customColors) as [
      string,
      ColorShadeMap,
    ][]) {
      for (const shade of colorShades) {
        variables[`--color-${colorName}-${shade}`] = shades[shade]
      }
    }
  }
 ...
 *   Values may reference built-in palette names, custom color names, or any
 *   string matching a `--color-<name>-<shade>` CSS variable.
 * @returns A record mapping CSS variable names to their values
 *
 * @example
 * ```ts
 ...
 *
 * // With custom colors:
 * const vars2 = generateSemanticColorVariables({ primary: 'rust' })
 * // vars2['--color-primary-500'] === 'var(--color-rust-500)'
 * ```
 */
export function generateSemanticColorVariables<C extends string = never>(
  overrides?: SemanticColorOverrides<C>
): Record<string, string> {
  const variables = {} as Record<string, string>
  const semanticMap = resolveSemanticColorMap(
    overrides as SemanticColorOverrides | undefined
  )

  // semantic colors
  semanticColorNames.forEach(colorName => {
    const baseColor = normalizeColorName(
      semanticMap[colorName] as ThemeColorName,
      overrides as SemanticColorOverrides | undefined
    )
    colorShades.forEach(shade => {
      // Use string interpolation directly so custom color names that aren't
      // part of ThemeColorName still resolve to valid CSS variable references.
 ...
 * ```
 */
export function generateColorVariables<C extends string = never>(
  customColors?: Record<C, ColorShadeMap>
): Record<string, string> {
  return {
