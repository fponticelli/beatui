 ...
 * ```
 */
export const BareEditor = (options: BareEditorOptions): Renderable => {
  const {
    value,
    format,
    ...
    html.div(
      ...
      html.div(
        ...
        WithElement(container => {
          ...
          const mount = async () => {
            try {
              // Resolve plugin config (use 'bare' defaults if not provided)
              const resolvedPlugins: PluginConfig =
                pluginConfig ?? createDefaultPluginConfig('bare')
              ...
              disposers.push(
                editor.registerUpdateListener(
                  ({ editorState, dirtyElements, dirtyLeaves }) => {
                    // Only fire on actual content changes
              ...
              disposers.push(
                appearance.on(a => {
                  ...
                  if (root) {
                    // Theme is handled at the container level via CSS,
                    // but we can add/remove classes if needed
