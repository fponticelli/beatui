...
export function FieldsetContainer({
  ...
  return html.fieldset(
    ...
    html.div(
      ...
      Stack(attr.class('gap-1'), ...children)
    )
  )
}
...
export function GroupContainer({
  ...
  return Card(
    ...
    Stack(attr.class('gap-1'), title ? Label(title) : null, ...children)
  )
}
    ...
    typeof cols === 'number'
      ...
      : computedOf(cols)(colsConfig => {
          if (typeof colsConfig === 'number') {
            return `grid grid-cols-${colsConfig}`
          }
          if (colsConfig && typeof colsConfig === 'object') {
            const classes = ['grid']
            if (colsConfig.sm) classes.push(`sm:grid-cols-${colsConfig.sm}`)
            if (colsConfig.md) classes.push(`md:grid-cols-${colsConfig.md}`)
            if (colsConfig.lg) classes.push(`lg:grid-cols-${colsConfig.lg}`)
            if (colsConfig.xl) classes.push(`xl:grid-cols-${colsConfig.xl}`)
            if (colsConfig['2xl'])
              classes.push(`2xl:grid-cols-${colsConfig['2xl']}`)
            return classes.join(' ')
          }
          return 'grid grid-cols-1'
        })

  return html.div(attr.class(gridClasses), attr.class('gap-4'), ...children)
}
...
export function applyContainerLayout(
  ...
  if (!config || !config.format) {
    ...
    if (config?.order) {
      ...
      return Stack(attr.class('gap-1'), ...orderedChildren)
    }
    return Stack(attr.class('gap-1'), ...children)
  }
