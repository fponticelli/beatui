...
const es: BeatUIMessages = {
  ...
  paginationRange: (
    ...
    total > filtered
      ? `Filas ${from} a ${to} de ${filtered} de ${total}`
      : `Filas ${from} a ${to} de ${filtered}`,
  ...
  carousel: {
    ...
    slideOfTotal: (index: number, total: number) =>
      `Diapositiva ${index} de ${total}`,
    goToSlide: (index: number) => `Ir a la diapositiva ${index}`,
  },
  // Date picker
  datePicker: {
    ...
    previousYears: (count: number) => `${count} años anteriores`,
    nextYears: (count: number) => `${count} años siguientes`,
    dateRangePickerLabel: 'Selector de rango de fechas',
    monthNames: [
      'Enero',
      'Febrero',
      'Marzo',
  ...
  timePicker: {
    ...
    selectSeconds: 'Seleccionar segundos',
    selectPeriod: 'Seleccionar AM/PM',
    nowLabel: 'Ahora',
  },
  // Time select
  ...
  transferList: {
    ...
    searchAvailable: 'Buscar elementos disponibles…',
    searchSelected: 'Buscar elementos seleccionados…',
  },
  // Copy button
  copyButton: {
    copied: '¡Copiado!',
  ...
  scrollableGridView: 'Vista de cuadrícula desplazable',
  // Onboarding tour
  onboardingTour: {
    label: 'Tour guiado',
