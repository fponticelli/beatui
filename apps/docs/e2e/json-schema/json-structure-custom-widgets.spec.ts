...
test.describe('JSON Structure Custom Widgets Component', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/json-structure-custom-widgets')
    await page.waitForLoadState('networkidle')
    await page.locator('main').waitFor({ state: 'visible', timeout: 5000 })
  })

  test('should render the JSON structure custom widgets page', async ({
    page,
  }) => {
