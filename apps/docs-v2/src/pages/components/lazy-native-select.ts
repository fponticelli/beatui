...
async function loadCountries({ request, abortSignal }: { request: string; abortSignal: AbortSignal }): Promise<SelectOption<string>[]> {
    ...
    sections: [
      Section(
        ...
        () => {
          ...
          return html.div(
            attr.class('w-64 space-y-2'),
            html.p(attr.class('text-xs text-gray-500 dark:text-gray-400'), 'Options load after a 600ms simulated delay:'),
            LazyNativeSelect<string, string>({
              value,
              request,
              load: loadCountries,
        ...
        () => {
          ...
          return html.div(
            ...
            html.div(
              ...
              html.label(attr.class('text-sm text-gray-600 dark:text-gray-400 shrink-0'), 'Filter:'),
              html.input(
                attr.type('text'),
                attr.placeholder('e.g. United'),
                attr.class('flex-1 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300'),
                on.input((e: Event) => {
            ...
            html.p(attr.class('text-xs text-gray-400'), 'Changing the search filter reloads options.')
          )
        },
      ...
      Section(
        ...
        () =>
          html.div(
            attr.class('font-mono text-xs bg-gray-100 dark:bg-gray-800 rounded-lg p-4 space-y-2 text-gray-700 dark:text-gray-300'),
            html.div('LazyNativeSelect({'),
            html.div(attr.class('pl-4'), '// Standard InputOptions (value, onChange, size, color, disabled, ...)'),
            ...
            html.div(attr.class('pl-4'), 'load: ({ request, abortSignal }) => Promise<SelectOption<T>[]>,'),
            html.div(),
